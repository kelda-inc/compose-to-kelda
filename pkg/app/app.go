...
import (
	...
	"github.com/kelda-inc/compose-to-kelda/pkg/loader"
...
func ValidateFlags(bundle string, args []string, cmd *cobra.Command, opt *kobject.ConvertOptions) {
	...
	if len(bundle) > 0 {
		inputFormat = "bundle"
		log.Fatalf("DAB / bundle (--bundle | -b) is no longer supported. See issue: https://github.com/kelda-inc/compose-to-kelda/issues/390")
...
func Convert(opt kobject.ConvertOptions) {
	...
	for name, svc := range komposeObject.ServiceConfigs {
		...
		if err := writeService(name, fmt.Sprintf("kelda-config/example-%s.yaml", name)); err != nil {
			log.Fatalf("write example service config", err.Error())
		}
	}
...
func writeService(svcName, path string) error {
	text := fmt.Sprintf(`# Copy this file into the service's repository, and rename it to kelda.yaml.
# This config file implements a simple sync config for Node applications.
# It assumes that a package-lock.json exists locally (generated by running 'npm
# install' on the local machine).
version: 'v1alpha1'
name: '%s'

# Use this image rather than the production image when running in development mode.
image: 'casestack/node-service:10'

# After every file sync, run this command.
# This probably needs to be changed to fit your application.
command: ['node', 'index.js']

# Whenever 'package-lock.json' is changed, run this command.
initCommand: ['npm', 'ci']

sync:
  # Sync all the files in the local directory.
  - from: '.'
    to: '.'
    # Ignore node_modules and .git so that we're not syncing too many files.
    except: ['node_modules', '.git']

  # When package-lock.json or ~/.npmrc is synced, run the "initCommand", which is 'npm ci'.
